      ...
      try {
        ...
        if (storedRole === 'Instructor') {
          ...
          const problemDetails = await Promise.all(
            user.assignedProblems.map((pid: string) =>
              axios.get(`http://localhost:3000/problems/${pid}`).catch((err) => {
          ...
          const completedProblems = await Promise.all(
            user.completedProblems.map(
              (pid: string) => axios.get(`http://localhost:3000/problems/${pid}`).catch((err) => {
                if (err.response && err.response.status === 404) {
                  return null;
                } else {
                  throw err;
          ...
          problemDetails.concat(completedProblems);

          setProblems(problemDetails.map(response => response.data));
        }
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-3xl font-bold mb-4">
        {userRole === 'Student' ? 'Available Problems' : 'All Problems'}
      </h1>
